...
<div className={`${LoadedState && 'anim_fadeIn'} w-full task_home_page_info_block bg-[#1E1E2F] rounded-[16px] py-[10px] px-[15px] flex duration-[.2s] flex-col ${open == false ? 'gap-[0px]':'gap-[20px]'} `}>
  ...
  return <TaskWithFunc taskParticipantId={e?.taskParticipantId} readyCount={e?.readyCount} requiredReadyCount={e?.requiredReadyCount} id={e.id} phrase={e.comment} date={e.endTime} friends={e.participants ? e.participants :[]}  timeout={e.timeout} title={e.title} type={e.type=='SINGLE'?'Одиночное':'Совместное'} key={e.title}/>
        }
        else {
